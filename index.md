# nycdemog ![](reference/figures/nycdemog.png)
...
on the `county` column, which matches
...
that hard-code the five NYC counties, never download geometry, and
return a tidy tibble keyed on a lowercase `geoid`.
...
to browse the variable catalogues:
...
[`nycmaps::nyc_boros_sf`](https://rdrr.io/pkg/nycmaps/man/nyc_boros_sf.html)
on `county`. Mapping median household income across NYC tracts:
...
![Choropleth of NYC Census tracts shaded by median household
income.](reference/figures/README-map-1.png)
