...
The nycdemog package provides tract- and block-level demographic data for the five counties of New York City, derived from the US Census Bureau via [tidycensus](https://walker-data.com/tidycensus/). It ships a curated set of tibbles keyed by `geoid` covering race, age and sex, household structure, income and poverty, education, employment, housing, and language and nativity, alongside thin wrapper functions that make custom NYC-only Census queries painless.

All datasets join 1:1 to the corresponding `sf` objects in [nycmaps](https://github.com/kjhealy/nycmaps) on the `geoid` column.
...
The pre-built data tables work offline. The wrapper functions hit the Census API and require a key, set as the `CENSUS_API_KEY` environment variable.

## Stored datasets

Every shipped tibble has `geoid` and `county` as its first two columns. ACS estimates are paired with `_moe` margin-of-error columns.
