# Build nyc_tract_acs_language_nativity_df: language spoken at home and
# nativity / citizenship from the latest 5-year ACS, at the Census tract
...
raw <- cache_call(
  "nyc_tract_acs_language_nativity",
  get_nyc_acs(
...
nyc_tract_acs_language_nativity_df <- raw |>
  select(
    geoid,
    county,
    name,
...
attr(nyc_tract_acs_language_nativity_df, "acs_year") <- attr(raw, "acs_year")
attr(nyc_tract_acs_language_nativity_df, "acs_survey") <- attr(
  raw,
  "acs_survey"
)
